...
import logging
import time
from perp.hyperliquid.hyperliquid_types import L2BookSubscription, TradesSubscription, UserEventsSubscription, AllMidsSubscription, Subscription, WsMsg, ActiveSubscription
from typing import Optional, List, Tuple, Dict, Callable, Any
from perp.utils.funcs import extract_info_from_proxy_row
...
class WebsocketManager(threading.Thread):
    ...
    def run(self):
        ...
        if not self.proxy:
            ...
            proxy = extract_info_from_proxy_row(self.proxy)
            self.ws.run_forever(http_proxy_host=proxy["host"], proxy_type="http", http_proxy_port=proxy["port"], http_proxy_auth=(proxy["username"], proxy["password"]))
